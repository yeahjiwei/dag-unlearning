...
parser.add_argument(
    ...
    default=None,
    type=int,
    # 根据给定的分布，以优化的方式(PLS-GAP分区)将数据集分割到给定数量的分片中，创建相应的splitfile
...
parser.add_argument(
    ...
    default=None,
    type=int,
    # 根据给定的分布生成给定数量的取消学习请求，并将它们直接应用到splitfile中
...
parser.add_argument(
    "--distribution",
    default="uniform",
    # 与--shards一起使用时假设分布，与--requests一起使用时采样分布。使用'reset'重置请求文件，默认一致
...
# Load dataset metadata.(加载数据集元数据。)
with open(args.dataset) as f:
    datasetfile = json.loads(f.read())

if args.shards is not None:
    # If distribution is uniform, split without optimizing.(如果分布是均匀的，则不进行优化分割)
    ...
    # Else run PLS-GAP algorithm to find a low cost split.(否则运行PLS-GAP算法找到一个低成本的分割。)
    else:
        ...
        if args.shards != None:
            # Initialize queue and partition.(初始化队列和分区)
            ...
            # Put all points in the top queue.(把所有的点放在最上面的队列中)
            ...
            for _ in range(datasetfile["nb_train"] - args.shards):
                ...
                # Otherwise insert it in the bottom queue.(否则将其插入底部队列)
                else:
                    # Bottom queue is ordered by cost only.(底部队列仅按cost排序)
                    position_array = np.where(
            ...
            # Generate splitfile and empty request file.(生成splitfile和空请求文件)
            np.save("../containers/{}/splitfile.npy".format(args.container), partition)
...
if args.requests != None:
    if args.distribution == "reset":
        ...
        requests = []
        # Divide up the new requests among the shards.(在shards中划分新的请求)
